  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
        <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Current Period</p>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {dignityCaption(dashas.current_maha) && (
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{dashas.current_maha} is {dignityCaption(dashas.current_maha)}.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        <div className="relative h-10 rounded-full overflow-hidden border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          ...
          {/* Today marker */}
          <div
            className="absolute top-0 h-full w-0.5 bg-gray-900 dark:bg-white z-10"
            style={{ left: `${todayPct}%` }}
          >
            <div className="absolute -top-1 -translate-x-1/2 w-2 h-2 rounded-full bg-gray-900 dark:bg-white" />
          </div>
        </div>
        {/* Year labels */}
        <div className="flex justify-between mt-1 text-xs text-gray-500 dark:text-gray-400">
          <span>{birthYear}</span>
          <span className="text-gray-900 dark:text-gray-100 font-medium">Today</span>
        ...
        {dashas.mahadashas.map((maha, i) => {
          ...
          return (
            <div key={i} className={`rounded-lg border transition-all ${
              isSelected ? "border-gray-400 shadow-sm" : "border-gray-100 dark:border-gray-800"
            } ${isPast ? "opacity-50" : ""}`}>
              <button
                className="w-full flex items-center gap-3 p-3 text-left hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded-lg"
                onClick={() => loadPrediction(i)}
              >
                <span className="text-xl">{PLANET_META[maha.lord.toLowerCase() as PlanetKey]?.symbol ?? "●"}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-gray-900 dark:text-gray-100">{maha.lord} Maha Dasha</span>
                    {isCurrent && (
                      <span className="text-xs bg-gray-900 dark:bg-indigo-600 text-white px-2 py-0.5 rounded-full">Now</span>
                    )}
                  </div>
                ...
                <span className="text-gray-300 dark:text-gray-600">{isSelected ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</span>
              </button>

              {/* Prediction panel */}
              {isSelected && (
                <div className="px-3 pb-3 border-t border-gray-100 dark:border-gray-800 mt-0">
                  {!session ? (
                    <SignInRequired feature="AI dasha predictions" />
                  ) : loadingPrediction ? (
                    <p className="text-sm text-gray-500 dark:text-gray-400 py-4 text-center">Reading the planetary currents...</p>
                  ) : predictionError ? (
                    <p className="text-sm text-red-500 dark:text-red-400 py-3">{predictionError}</p>
                  ) : prediction ? (
                    <div className="pt-3 space-y-3">
                      <p className="text-sm text-gray-700 dark:text-gray-300 italic">{prediction.summary}</p>
                      <AdviceDisclaimer className="text-left" />
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">Core Themes</p>
                        ...
                        <div>
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">Cultivate</p>
                          <ul className="text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                            {prediction.cultivate?.map((t, j) => <li key={j} className="flex gap-1"><span>·</span>{t}</li>)}
                          </ul>
                        </div>
                        <div>
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">Watch Out For</p>
                          <ul className="text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                            {prediction.challenges?.map((t, j) => <li key={j} className="flex gap-1"><span>·</span>{t}</li>)}
                          </ul>
                        </div>
                        <div>
                          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">Action Areas</p>
                          <ul className="text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                      ...
                      {/* Antardasha list */}
                      <div>
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-2">Sub-Periods (Antardasha)</p>
                        <div className="space-y-1.5">
                          {maha.antardashas.map((a, j) => {
                            ...
                            return (
                              <div key={j} className="border border-gray-100 dark:border-gray-800 rounded">
                                <button
                                  ...
                                  className={`w-full text-left text-xs px-2 py-1.5 rounded flex items-center justify-between ${
                                    isCurrentAntar ? "bg-gray-900 dark:bg-indigo-600 text-white" : "bg-gray-50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800"
                                ...
                                >
                                  ...
                                  {hasPratyantars && (
                                    <span className={isCurrentAntar ? "text-white" : "text-gray-500 dark:text-gray-400"}>
                                ...
                                {isAntarExpanded && hasPratyantars && (
                                  <div className="px-2 pb-2 pt-1 bg-gray-50 dark:bg-gray-800/50">
                                    <p className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">Pratyantar Dasha</p>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 py-2">Click to generate prediction</p>
                  )}
                </div>
              )}
            </div>
