...
14. ~~**Surface skeptic-mode framing as a pre-onboarding product moment, not a buried settings toggle**~~ — **shipped** *(promoted from §5.4 Candidates 2026-07-31, shipped same day)*. A static, hand-authored sample reading (`lib/sampleFoundation.ts`) now shows right after the hero — before the trust-signal feature cards, before any signup — with a jyotish/plain-language toggle reusing the existing `tonePreference` state. Bundled with removing the home page's page-load auth redirect (unauthenticated visitors can now view the hero/features/sample demo; chart *submission* still requires sign-in per §4.1, the gate just moved to submit-time), which also closes the external critique's item #1 (taste-before-data) — a first-time visitor never reached any of this content before, since the whole page redirected to sign-in before rendering.
