...
14. **Surface skeptic-mode framing as a pre-onboarding product moment, not a buried settings toggle** — *(promoted from §5.4 Candidates, approved 2026-07-31)*. Today `tonePreference` only becomes visible on the Coach screen, after a user has already entered a Jyotish-fluent chart flow — the audience §2.2/§3 name as the single biggest open lane (The Pattern's 15M users) never encounters it. A landing-page-level choice ("I don't really believe in this — just show me the personality read") before/alongside chart entry, using the *same* underlying `tonePreference` mechanism that already exists — no new reasoning path, just earlier exposure of the existing one.
