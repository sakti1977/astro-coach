    ...
    <div className="max-w-lg mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4">
        <button
          ...
          className="flex items-center justify-center gap-2 py-4 rounded-xl border-2 border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-medium text-lg hover:border-red-300 dark:hover:border-red-700 hover:bg-red-50 dark:hover:bg-red-950/40 hover:text-red-700 dark:hover:text-red-300 transition-all active:scale-95"
        >
          <X className="w-5 h-5" /> No
        </button>
        <button
          onClick={() => answer(true)}
          className="flex items-center justify-center gap-2 py-4 rounded-xl border-2 border-indigo-600 bg-indigo-600 text-white font-medium text-lg hover:bg-indigo-700 transition-all active:scale-95"
        >
      ...
      {answers.length > 0 && (
        <div className="mt-8">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-3">Your answers so far</p>
            ...
            {[...answers].reverse().map((a, i) => (
              <div key={i} className="flex items-start gap-2 text-sm">
                <span className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center
                  ${a.answer ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400"}`}>
                  {a.answer ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                </span>
                <span className="text-gray-600 dark:text-gray-400">{a.question.question}</span>
              </div>
            ))}
          </div>
        </div>
